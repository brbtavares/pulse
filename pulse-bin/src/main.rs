...
async fn run_pipeline(path: std::path::PathBuf) -> anyhow::Result<()> {
    ...
    let op_window = match cfg.window.kind.as_str() {
        "tumbling" => match agg {
            ...
            "sum" => pulse_ops::WindowedAggregate::tumbling_sum(
                key_field.clone(),
                win_size_ms,
                agg_field.clone().unwrap(),
            ),
            "avg" => pulse_ops::WindowedAggregate::tumbling_avg(
                key_field.clone(),
                win_size_ms,
                agg_field.clone().unwrap(),
            ),
            "distinct" => pulse_ops::WindowedAggregate::tumbling_distinct(
                key_field.clone(),
                win_size_ms,
                agg_field.clone().unwrap(),
            ),
        ...
        "sliding" => {
            let slide_ms = pulse_core::config::parse_duration_ms(
                cfg.window
                    .slide
                    .as_deref()
                    .ok_or_else(|| anyhow::anyhow!("slide must be set for sliding window"))?,
            )?;
            match agg {
                "count" => {
                    pulse_ops::WindowedAggregate::sliding_count(key_field.clone(), win_size_ms, slide_ms)
                }
                "sum" => pulse_ops::WindowedAggregate::sliding_sum(
                    key_field.clone(),
                    win_size_ms,
                    slide_ms,
                    agg_field.clone().unwrap(),
                ),
                "avg" => pulse_ops::WindowedAggregate::sliding_avg(
                    key_field.clone(),
                    win_size_ms,
                    slide_ms,
                    agg_field.clone().unwrap(),
                ),
                "distinct" => pulse_ops::WindowedAggregate::sliding_distinct(
                    key_field.clone(),
                    win_size_ms,
                    slide_ms,
                    agg_field.clone().unwrap(),
        ...
        "session" => {
            let gap_ms = pulse_core::config::parse_duration_ms(
                cfg.window
                    .gap
                    .as_deref()
                    .ok_or_else(|| anyhow::anyhow!("gap must be set for session window"))?,
            )?;
            match agg {
                "count" => pulse_ops::WindowedAggregate::session_count(key_field.clone(), gap_ms),
                "sum" => pulse_ops::WindowedAggregate::session_sum(
                    key_field.clone(),
                    gap_ms,
                    agg_field.clone().unwrap(),
                ),
                "avg" => pulse_ops::WindowedAggregate::session_avg(
                    key_field.clone(),
                    gap_ms,
                    agg_field.clone().unwrap(),
                ),
                "distinct" => pulse_ops::WindowedAggregate::session_distinct(
                    key_field.clone(),
                    gap_ms,
                    agg_field.clone().unwrap(),
                ),
    ...
    let src: Box<dyn pulse_core::Source> = match cfg.source.kind.as_str() {
        "file" => match cfg.source.format.as_deref().unwrap_or("jsonl") {
            "jsonl" => Box::new(pulse_io::FileSource::jsonl(
                cfg.source.path.to_string_lossy(),
                cfg.source.time_field.clone(),
            )),
            "csv" => Box::new(pulse_io::FileSource {
                path: cfg.source.path.to_string_lossy().to_string(),
                format: pulse_io::FileFormat::Csv,
                event_time_field: cfg.source.time_field.clone(),
                text_field: None,
            }),
        ...
        "kafka" => {
            ...
            if let Some(ms) = cfg.source.commit_interval_ms {
                s.commit_interval = std::time::Duration::from_millis(ms);
            }
            Box::new(s)
        }
...
mod tests {
    ...
    async fn golden_simple_pipeline_parquet_rowcount() {
        ...
        if let Some(parent) = tmp_cfg.parent() {
            let _ = std::fs::create_dir_all(parent);
        }
        tokio::fs::write(&tmp_cfg, toml::to_string(&cfg).unwrap())
            .await
            .unwrap();
    ...
    async fn golden_tumbling_sum_file_sink() {
        ...
        let sum_a = vals.iter().find(|v| v["key"] == "a").unwrap()["sum"]
            .as_f64()
            .unwrap();
        let sum_b = vals.iter().find(|v| v["key"] == "b").unwrap()["sum"]
            .as_f64()
            .unwrap();
        assert_eq!(sum_a, 3.0);
        assert_eq!(sum_b, 3.0);
    ...
    async fn golden_tumbling_distinct_file_sink() {
        ...
        assert_eq!(lines.len(), 2, "two keys a and b");
        let vals: Vec<serde_json::Value> = lines.iter().map(|l| serde_json::from_str(l).unwrap()).collect();
        let d_a = vals.iter().find(|v| v["key"] == "a").unwrap()["distinct_count"]
            .as_i64()
            .unwrap();
        let d_b = vals.iter().find(|v| v["key"] == "b").unwrap()["distinct_count"]
            .as_i64()
            .unwrap();
        assert_eq!(d_a, 1);
        assert_eq!(d_b, 1);
    ...
    async fn csv_source_tumbling_count_file_sink() {
        ...
        assert_eq!(lines.len(), 2);
        let vals: Vec<serde_json::Value> = lines.iter().map(|l| serde_json::from_str(l).unwrap()).collect();
        let c_a = vals.iter().find(|v| v["key"] == "a").unwrap()["count"]
            .as_i64()
            .unwrap();
        let c_b = vals.iter().find(|v| v["key"] == "b").unwrap()["count"]
            .as_i64()
            .unwrap();
        assert_eq!(c_a, 2);
        assert_eq!(c_b, 1);
