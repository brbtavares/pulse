...
impl Context for ChanCtx {
    ...
    fn kv(&self) -> std::sync::Arc<dyn KvState> { self.kv.clone() }
    fn timers(&self) -> std::sync::Arc<dyn Timers> { std::sync::Arc::new(TestTimers) }
}

#[tokio::test]
async fn kafka_roundtrip_basic_if_env_present() {
    let brokers = match std::env::var("KAFKA_BROKER") { Ok(v) => v, Err(_) => return }; // skip if not configured
    let topic = match std::env::var("KAFKA_TOPIC") { Ok(v) => v, Err(_) => return };

    // Produce a couple of JSON messages
    let producer: FutureProducer = ClientConfig::new().set("bootstrap.servers", &brokers).create().expect("producer");
    for i in 0..2 {
        ...
        let _ = producer.send(FutureRecord::to(&topic).payload(&payload), Duration::from_secs(1)).await;
    }

    // Run KafkaSource and collect a few outputs, then abort
    let mut src = KafkaSource::new(brokers, format!("g-{}", std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos()), topic, "event_time");
    ...
    let handle = tokio::spawn(async move { let _ = src.run(&mut ctx).await; });
